...
4. **`sessions.delete` 不带 archivedOnly**：`SessionsDeleteParamsSchema.archivedOnly` 注释——*"operator.write callers must set this; deletes without it require operator.admin."* 但实测校准（#370 三轮 P0）：官方 webchat（`sessions-page.ts`）**仅当 `row.archived === true` 才带 archivedOnly**，且网关对未归档会话带 archivedOnly:true 直接 `INVALID_REQUEST`（"Session X is not archived. Archive it first, then delete it."）——面板从不先归档，恒带 archivedOnly 会让**所有**正常会话删除失败。旧 wire（`wire_client.py` `sessions.delete {key}`）也不带，缺失会话为 `ok` 无操作（幂等）。**故删除不带 archivedOnly**。真网关实测（#461 / ADR 0006 实测项③，2026.7.1-browser）：webchat 客户端（`webchat-ui` 或 `mode=webchat`）的 `sessions.delete/patch/compact/restore` 被 `rejectWebchatSessionMutation` **硬拒**（"webchat clients cannot delete sessions; use chat.send for session-scoped updates"），豁免仅 `client.id === 'openclaw-control-ui'`（官方 control-ui 页面身份）——**障碍是客户端身份而非 scope**（面板 `OPERATOR_SCOPES` 已含 admin，scope 校验通过）。面板客户端 ID 从 `webchat-ui` 改为 `openclaw-control-ui`（#461）后删除可用；配对不受影响（网关 `BROWSER_DEVICE_CLIENT_IDS` 含 control-ui，生产远程非 loopback 仍要求配对）。
