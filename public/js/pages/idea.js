/* ===== Idea Page ===== */
var ideaDomains = [];
var ideaEvalFilter = 0; // 0=all domains
...
pages.idea = async function(){
  ...
  if(cache.ideas.length>0){
    ...
    // domain 过滤
    if(Object.keys(usedDomains).length>1){
      h+='<div style="display:flex;align-items:center;gap:10px;margin-bottom:16px">';
      h+='<span style="font-size:10px;color:#464d65;flex-shrink:0">筛选知识库</span>';
      h+='<select class="inp" id="ideaEvalDomain" onchange="onIdeaEvalFilter()" style="font-size:12px;max-width:240px">'+domainOpts+'</select>';
      h+='</div>';
    }
    ...
    sorted.forEach(function(idea, idx){
      ...
      h+='<span style="width:26px;height:26px;border-radius:7px;background:'+(idx===0?'#A78BFA':'rgba(255,255,255,.05)')+';display:grid;place-items:center;font-family:Space Grotesk,sans-serif;font-weight:700;font-size:12px;color:'+(idx===0?'#fff':'#464d65')+';flex-shrink:0">'+(idx+1)+'</span>';
      ...
      h+='<span style="font-family:Space Grotesk,sans-serif;font-weight:700;font-size:18px;color:'+sc+';flex-shrink:0">'+idea.os.toFixed(1)+'</span></div>';
      ...
      h+='<span style="font-size:11px;color:#7d849a">来源：'+esc(idea.fp||'--')+'</span></div>';

      // 描述
      h+='<div style="font-size:12px;color:#7d849a;margin-bottom:10px;line-height:1.5">'+esc(idea.desc)+'</div>';
    ...
    if(sorted.length===0){
      h+='<div style="text-align:center;padding:30px;color:#464d65;font-size:12px">该知识库暂无 Idea 记录</div>';
    }
    h+='</div>';
  }
  ...
  // 无数据提示
  if(cache.ideas.length===0 && ideaDomains.length>0){
    h+='<div class="card" style="text-align:center;padding:40px;color:#464d65"><i class="fa-solid fa-lightbulb" style="font-size:30px;display:block;margin-bottom:10px;opacity:.2"></i><p style="font-size:12px">暂无已生成的 Idea，请在上方选择知识库和问题后开始生成</p></div>';
  }
