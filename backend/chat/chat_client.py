...
# #229：wire 异常族 / GatewayPolicy / 握手默认常量迁入 integration.openclaw.wire（防腐层单源），
# 本模块 identity-preserving re-export（同对象，非拷贝）——isinstance / is / __cause__ 链不变，
# 既有 import 与 except 子句零改动。两个 DEFAULT_* 常量本模块不再直接用（仅 GatewayPolicy 用，
# 已随之迁走），保留为公开 re-export。
from integration.openclaw.wire import (
    DEFAULT_MAX_PAYLOAD_BYTES,  # noqa: F401  — #229 identity re-export（本模块不再直接用）
    DEFAULT_TICK_INTERVAL_MS,  # noqa: F401  — #229 identity re-export（本模块不再直接用）
    ChatClientError,
    ChatConnectError,
