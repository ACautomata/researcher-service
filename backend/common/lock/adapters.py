...
class SyncRedisLockAdapter:
    """``DistributedLock`` Port 的真 Redis **sync** Adapter（issue #254 / parent #243）。

    #254 裁决（T4）：sync 调用点（orchestrator create / pairing threadpool）不经
    async adapter 桥接，而是**同一 DistributedLock Port 的 sync 实现**——threadpool 内
    嵌套事件环（async_to_sync 起新 loop 于 executor 线程）与 redis.asyncio Lock 的
    loop-bound 是硬约束，不能假装一个 asyncio Lock 无差别服务 sync 线程（#247 D3）。

    **阻塞语义**：阻塞 acquire 交给 redis-py 同步 ``Lock.acquire(blocking=True)``——
    它已把「SET NX PX + sleep 轮询」封装进锁对象（含 TTL 自动过期、Lua CAS 释放），
    sync 线程无需自己拼 Event/轮询。租约折叠进 TTL+renew（不设独立 Lease 子类型）。

    **注入式 client**：收一个 ``redis.Redis``-compatible client（只用 ``lock()`` 工厂面），
    由 ``LockFleet`` 懒共享注入；本模块自身**不连网**——``lock()`` 只是构造 Lock 对象，
    首个 Redis 命令（acquire 的 SET NX PX）才触网。
    """
    ...
    def _key_for(resource: LockResource) -> str:
        ...
        与 async adapter 共用 ``_KEY_BUILDERS`` 闭合表——未登记的变体（裸 string /
        外来类）抛 ``TypeError``（防 KV 逃逸口 #246 Q6）。
        """
        build = _KEY_BUILDERS.get(type(resource))
        if build is None:
            variants = '/'.join(v.__name__ for v in _KEY_BUILDERS)
...
class SyncRedisLeaseHandle:
    """LeaseHandle Port 的 Redis sync 实现（compose 一个 redis.Redis Lock 持有方 token）。

    续约/释放转发到底层 Lock 的 Lua CAS 原语（与 ``_RedisLeaseHandle`` 同一语义，
    sync 形态仅把 ``await`` 换成阻塞调用）：
    - ``renew(new_ttl)`` → ``extend(new_ttl, replace_ttl=True)`` 仍持有才重置 TTL；
      过期/被顶替抛 ``LockNotOwnedError``，吞掉成 no-op（不 resurrect 锁）；
    - ``release()`` → ``release()`` Lua 安全释放（CAS 比对 token，只释放自己的锁）；
      过期被顶替抛 ``LockNotOwnedError``，吞掉成幂等 no-op。幂等由本 handle 的
      ``_released`` 标记保证二次调用不再触网。
    """
    ...
    def renew(self, new_ttl: timedelta) -> None:
        ...
        try:
            ...
            # 锁已过期 / 已被他人顶替 / 已不持有：不 resurrect（对齐 FakeLockSync 语义）。
            pass
