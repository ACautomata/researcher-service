...
#278 预重构（parent #277）：本文件由原 ``containers.orchestrator.py`` 模块**原样迁移**为
``containers/fleet/`` 子包（包名无下划线，符合「包名禁下划线」约定；issue #278）——
``from containers.orchestrator import ...`` 的 import 路径不变（原模块退为 re-export 薄壳，
双路径可达），解析到同一模块对象、同对象语义（``is`` / isinstance / 异常 ``__cause__`` 链）
完全保持。
...
from containers.ports import RESERVED_PORT_18789, PortAllocator
