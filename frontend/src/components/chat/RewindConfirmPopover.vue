...
// #694 回退确认 popover（#682 spec §1.5 / #685 官方 chat-confirm-popover 同构的哑组件）。
...
// 翻到下方。只挂载时量一次：popover 与锚点同属一个滚动内容，滚动时相对关系不变。
...
onMounted(() => {
  ...
  if (el && anchorEl && host) {
    const roomAbove = anchorEl.getBoundingClientRect().top - host.getBoundingClientRect().top
    placedBelow.value = roomAbove < el.offsetHeight + GAP_PX
  }
})
onBeforeUnmount(() => {
...
/* 卡片定位与观感对齐官方：气泡底、1px 边框、radius 8px、投影。 */
.rewind-confirm { width: 232px; padding: 12px; background: var(--el-bg-color-overlay); border: 1px solid var(--el-border-color); border-radius: 8px; box-shadow: 0 8px 24px rgba(0, 0, 0, .35); text-align: left; }
