...
对端网关是否支持回退 / fork / 分支这族 RPC 的判定：握手快照 `hello-ok.features.methods` 含全部四个方法名才算**可用**，否则面板整体隐藏这些入口（过渡期存量旧镜像容器混部时防呆——不出现点了必然报错的按钮）。能力随每次握手刷新（重连到旧网关即如实撤销）。**不新增状态**：入口的忙碌门复用既有的流式 / 连接中 / 已断线三态；回退**自身在途**（破坏性 RPC + 重建管线，单飞到落地为止）不进这道门——同一时刻只允许一次回退在途（两次在途会在网关侧竞争同一活跃路径、且落地序由网络决定，可能回填的不是最后一次意图），窗口期内再次触发由编排层静默忽略（无提示）。窗口 = 一次 RPC + 一次全量重拉：**RPC 阶段**消息投影未动，入口照常渲染可点（点到的被吞掉，慢网关下该阶段就是一次完整往返）；**重拉阶段**投影整体换新（先清空再铺底），本无可点。落地后入口立即恢复。
