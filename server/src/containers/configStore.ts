...
// 唯一落盘 seam：bytes-agnostic（只管「把一段 JSON 文本原子放到 instances/<name>/openclaw.json」）。
...
export class ConfigStore {
  ...
  // 把 payload（JSON 文本）原子写到 instances/<name>/openclaw.json，返回落地路径。
  async write(name: string, payload: string): Promise<string> {
    const configPath = path.join(this.config.root, 'instances', name, 'openclaw.json')
    const tmp = path.join(
